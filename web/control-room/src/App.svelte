<script>
  ...
  let timer;
  const controlRoutes = ['/v1/control/nodes', '/v1/control/workers', '/v1/control/clients', '/v1/control/projects', '/v1/control/diagnostics/'];
  ...
  async function saveConfig() {
    ...
    try { await request('/v1/control/config', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: configText }) }); await loadConfig(); await refresh(); }
    catch (reason) { error = reason.message; }
    actionInFlight = '';
  }
  ...
  async function saveProfile() {
    ...
    try { await request(`/v1/control/profiles/${encodeURIComponent(selectedProfile)}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: profileText }) }); await loadProfiles(); await refresh(); }
    catch (reason) { error = reason.message; }
    actionInFlight = '';
  }
...
<main class="min-h-screen bg-[#070b14] text-slate-100">
  <div class="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
    ...
    <nav class="mt-5 flex gap-1 overflow-x-auto rounded-2xl border border-white/10 bg-white/[0.03] p-1" aria-label="Control Room sections">
      ...
      <button class="ml-auto shrink-0 rounded-xl px-4 py-2 text-xs text-slate-400 hover:bg-white/10" on:click={refresh}>Refresh</button>
    </nav>
    ...
    {#if loading}<div class="mt-12 text-center text-sm uppercase tracking-[0.16em] text-slate-600">Reading durable state...</div>{:else if tab === 'overview'}
      <section class="mt-6 space-y-5">
        ...
        <div class="grid gap-5 lg:grid-cols-2"><section><h2 class="text-sm font-medium text-slate-300">Nodes and harness inventory</h2><div class="mt-3 space-y-3">{#each overview.nodes || [] as node}<article class="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><div class="flex justify-between gap-3"><h3 class="font-mono text-sm text-cyan-200">{node.node}</h3><span class="text-xs text-slate-400">{node.health}</span></div><p class="mt-2 text-xs text-slate-500">heartbeat {node.last_heartbeat_at || 'not observed'} · capacity {node.capacity}</p>{#each node.inventory?.instances || [] as instance}<div class="mt-3 rounded-xl bg-black/20 p-3 text-xs"><div class="flex justify-between"><span>{instance.harness} {instance.version}</span><span>{instance.status}</span></div><p class="mt-1 text-slate-500">{instance.id} · models {instance.model_ids?.join(', ') || 'none'}</p></div>{/each}<button class="mt-3 rounded-xl border border-rose-400/20 px-3 py-2 text-xs text-rose-300 hover:bg-rose-400/10 disabled:opacity-50" disabled={node.revoked || actionInFlight !== ''} on:click={() => revoke('nodes', node.node)}>Revoke Node</button></article>{/each}</div></section>
          <section><h2 class="text-sm font-medium text-slate-300">Workers, Turns and Attempts</h2><div class="mt-3 space-y-3">{#each overview.workers || [] as worker}<article class="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><div class="flex justify-between gap-3"><h3 class="text-sm text-slate-200">{worker.title}</h3><span class="text-xs text-slate-400">{worker.status}</span></div><p class="mt-2 font-mono text-xs text-cyan-200">{worker.worker_ref} · {worker.node} · {worker.harness_instance}</p><p class="mt-2 text-xs text-slate-500">recovery {worker.recovery?.state} · health {worker.recovery?.health}</p><div class="mt-3 flex flex-wrap gap-2">{#each worker.attempts || [] as attempt}<span class="rounded-lg border border-white/10 px-2 py-1 text-[11px] text-slate-400">Attempt {attempt.number}: {attempt.state}</span>{/each}<button class="rounded-lg border border-cyan-300/20 px-2 py-1 text-[11px] text-cyan-200 hover:bg-cyan-300/10" on:click={() => openDiagnostics(worker.worker_ref)}>Diagnostics</button></div>{#if worker.results?.length}<pre class="mt-3 whitespace-pre-wrap text-xs text-slate-400">{worker.results[worker.results.length - 1].summary}</pre>{/if}</article>{/each}</div></section></div>
      </section>
    {:else if tab === 'events'}
    ...
    {:else if tab === 'profiles'}
      <section class="mt-6 grid gap-5 lg:grid-cols-[15rem_minmax(0,1fr)]"><aside class="rounded-2xl border border-white/10 bg-white/[0.025] p-3"><h2 class="px-2 py-2 text-sm">Profiles</h2>{#each profiles as profile}<button class="w-full rounded-xl px-3 py-3 text-left text-xs {selectedProfile === profile.name ? 'bg-cyan-300 text-slate-950' : 'text-slate-400 hover:bg-white/10'}" on:click={() => selectProfile(profile.name)}>{profile.name}</button>{/each}</aside><div><div class="flex justify-between"><h2 class="text-sm">{selectedProfile || 'Profile'}</h2><button class="text-xs text-cyan-200" on:click={reloadProfiles}>Reload disk</button></div><textarea bind:value={profileText} spellcheck="false" class="mt-4 min-h-[28rem] w-full rounded-2xl border border-white/10 bg-black/25 p-4 font-mono text-xs text-slate-300"></textarea><button class="mt-3 rounded-xl bg-cyan-300 px-4 py-2 text-xs font-medium text-slate-950" disabled={!selectedProfile || actionInFlight !== ''} on:click={saveProfile}>Validate and apply</button></div></section>
    {:else if tab === 'config'}
      <section class="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]"><div><div class="flex justify-between"><h2 class="text-sm">Managed config</h2><button class="text-xs text-cyan-200" on:click={reloadConfig}>Reload disk</button></div><textarea bind:value={configText} spellcheck="false" class="mt-4 min-h-[28rem] w-full rounded-2xl border border-white/10 bg-black/25 p-4 font-mono text-xs text-slate-300"></textarea><button class="mt-3 rounded-xl bg-cyan-300 px-4 py-2 text-xs font-medium text-slate-950" disabled={actionInFlight !== ''} on:click={saveConfig}>Validate and apply</button></div><aside class="rounded-2xl border border-white/10 bg-white/[0.025] p-4"><h2 class="text-sm">Compiled snapshot</h2><pre class="mt-3 max-h-[34rem] overflow-auto whitespace-pre-wrap text-xs text-slate-500">{pretty(config?.snapshot || {})}</pre></aside></section>
    {/if}
  </div>
</main>
