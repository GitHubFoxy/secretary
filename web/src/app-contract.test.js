...
test('Svelte app compiles and keeps the Worker-first API seams', () => {
  ...
  assert.match(source, /visibleConversationEntries/);
  assert.match(source, /#each visibleEntries as entry/);
  ...
  assert.match(source, /approval\.kind === 'input'/);
  assert.match(source, /workerAction\('message', inputDrafts\[request\.requestId\].*request\.requestId/);
