<script>
  ...
  import { formatActivityPayload, formatWorkerStatus, inputRequest, mergeSequenced, secretaryEventText, visibleConversationEntries, workerActionBody, workerCard } from './ui-model.js';
  ...
  async function refreshObserver(workerRef) {
    try {
      const [details, diagnostics] = await Promise.all([
        request(`/v1/workers/${encodeURIComponent(workerRef)}`),
        request(`/v1/workers/${encodeURIComponent(workerRef)}/diagnostics`),
      ]);
      if (observer?.workerRef === workerRef) observer = { ...observer, details, diagnostics };
  ...
  async function openWorker(workerRef) {
    ...
    activityCursor = 0;
    try {
      const [details, replay, diagnostics] = await Promise.all([
        request(`/v1/workers/${encodeURIComponent(workerRef)}`),
      ...
      observer = { workerRef, details, diagnostics, loading: false };
  ...
  function observerHasTerminalTurn() {
    return ['succeeded', 'failed', 'canceled', 'interrupted'].includes(currentTurn?.state);
  }
  ...
  function connectActivity(workerRef, afterSeq = activityCursor) {
    ...
    activitySocket.onclose = () => {
      if (observer?.workerRef !== workerRef || observerHasTerminalTurn()) return;
      clearTimeout(activityReconnectTimer);
  ...
  function inputFieldValue(requestID, name, property) {
    const draft = inputFormDrafts[requestID]?.[name];
    if (draft !== undefined) return draft;
    if (property.default !== undefined) return property.default;
    return property.type === 'boolean' ? false : property.type === 'array' ? [] : '';
  }

  function setInputField(requestID, name, property, event) {
    let value;
    if (property.type === 'boolean') value = event.currentTarget.checked;
    else if (property.type === 'array') value = [...event.currentTarget.selectedOptions].map((option) => option.value);
    else if (property.type === 'number' || property.type === 'integer') value = event.currentTarget.value === '' ? '' : Number(event.currentTarget.value);
    else value = event.currentTarget.value;
    inputFormDrafts = { ...inputFormDrafts, [requestID]: { ...(inputFormDrafts[requestID] || {}), [name]: value } };
  }

  function submitInputRequest(request) {
    const values = { ...(inputFormDrafts[request.requestId] || {}) };
    const required = request.schema?.required || [];
    for (const name of required) {
      if (values[name] === undefined || values[name] === '' || values[name] === null) {
        observerError = `Input required: ${name}`;
        return;
      }
    }
    workerAction('message', JSON.stringify(values), request.requestId);
  }

  async function workerAction(action, text = '', requestID = '') {
    if (!observer?.workerRef || !text.trim() && action === 'message') return;
    observerError = '';
    const actionID = `${observer.workerRef}:${action}:${requestID}:${text}`;
    ...
    try {
      const response = await request(`/v1/workers/${encodeURIComponent(observer.workerRef)}/${action}`, {
        ...
        body: JSON.stringify(workerActionBody(text, requestID)),
      });
      ...
      if (action === 'message') {
        steerText = '';
        followUpText = '';
        if (requestID) inputDrafts = { ...inputDrafts, [requestID]: '' };
      }
      await refreshState();
  ...
  async function approve(approval) {
    if (!approval?.request_id || !observer?.workerRef) return;
    observerError = '';
    const actionID = `approval:${approval.request_id}`;
    pendingActions[actionID] ||= crypto.randomUUID();
    try {
      const response = await request(`/v1/workers/${encodeURIComponent(observer.workerRef)}/approve`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': pendingActions[actionID] },
        body: JSON.stringify({ request_id: approval.request_id, text: 'approve' }),
      });
      delete pendingActions[actionID];
      if (response?.worker) observer = { ...observer, details: response };
      await refreshState();
    ...
    <div class="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 pb-32 pt-4 sm:px-6 lg:px-8">
      ...
      <div class="mt-5 grid flex-1 gap-5 lg:grid-cols-[minmax(0,1fr)_24rem]">
        ...
        <aside class="min-w-0 space-y-5">
          <section><div class="mb-3 flex items-center justify-between"><h2 class="text-sm font-medium text-slate-300">Workers</h2><span class="text-xs text-slate-600">{workerCards.length}</span></div>{#if observerError && !observer}<p class="mb-3 rounded-xl bg-rose-400/10 p-3 text-xs text-rose-200">{observerError}</p>{/if}<div class="space-y-3">{#if workerCards.length === 0}<div class="rounded-2xl border border-dashed border-white/10 p-5 text-sm text-slate-500">Workers will appear here after Secretary accepts delegated work.</div>{/if}{#each workerCards as card (card.worker.worker_ref)}<article class="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><div class="flex items-start justify-between gap-3"><h3 class="line-clamp-3 text-sm leading-6 text-slate-200">{card.worker.title || card.worker.intent}</h3><span class="shrink-0 rounded-full border px-2 py-1 text-[10px] uppercase tracking-wide {statusTone(card.status)}">{card.statusLabel}</span></div><div class="mt-3 grid grid-cols-2 gap-1 text-[11px] text-slate-500"><span>Project: {card.project}</span><span>Node: {card.node}</span><span>Turn: {card.turnStatusLabel}</span><span>Worker: {card.acknowledgement}</span><span class="col-span-2">HarnessInstance: {card.harness}</span></div>{#if card.result}<p class="mt-3 whitespace-pre-wrap text-xs leading-5 text-emerald-200">Result: {card.result}</p>{/if}{#if card.nodeState !== 'online'}<p class="mt-2 text-xs {card.nodeState === 'blocked' ? 'text-rose-300' : 'text-amber-300'}">{card.nodeState === 'blocked' ? 'Blocked: binding is immutable' : 'Offline: waiting for the bound Node'}</p>{/if}<div class="mt-4 flex items-center justify-between gap-2 text-[11px] text-slate-600"><time>{formatDate(card.worker.created_at)}</time><button class="text-indigo-300 hover:text-indigo-200" on:click={() => openWorker(card.worker.worker_ref)}>Observe</button></div></article>{/each}</div></section>
      ...
      {#if observer}<div class="fixed inset-0 z-30 bg-black/60 lg:hidden" role="presentation" on:click={closeObserver}></div><section class="fixed inset-0 z-40 h-full max-h-none overflow-y-auto rounded-none border border-white/10 bg-[#111827] p-5 shadow-2xl shadow-black/50 lg:absolute lg:inset-auto lg:right-6 lg:top-24 lg:block lg:h-auto lg:max-h-[calc(100vh-8rem)] lg:w-[30rem] lg:rounded-3xl" aria-label="Worker observer"><div class="flex items-start justify-between gap-3"><div><p class="text-xs uppercase tracking-[0.16em] text-indigo-300">Worker observer</p><h2 class="mt-1 break-all text-sm font-medium text-slate-200">{observer.workerRef}</h2></div><button class="rounded-xl px-2 py-1 text-slate-400 hover:bg-white/10 hover:text-white" aria-label="Close observer" on:click={closeObserver}>×</button></div>{#if observer.loading}<p class="mt-8 text-sm text-slate-500">Loading durable Worker state...</p>{:else if observer.details}<div class="mt-5 flex flex-wrap items-center gap-2"><span class="rounded-full border px-2 py-1 text-[10px] uppercase tracking-wide {statusTone(observer.details.worker.status)}">{formatWorkerStatus(observer.details.worker.status)}</span>{#if currentTurn}<span class="rounded-full border border-white/10 px-2 py-1 text-[10px] uppercase tracking-wide text-slate-400">Turn: {formatWorkerStatus(currentTurn.state)}</span>{/if}<button class="ml-auto text-xs text-rose-300 hover:text-rose-200" on:click={() => workerAction('cancel')}>Stop</button><button class="text-xs text-slate-300 hover:text-white" on:click={() => workerAction('close')}>Close</button></div><div class="mt-4 grid grid-cols-2 gap-2 rounded-2xl bg-black/25 p-3 text-[11px] text-slate-500"><span>Project: {observer.details.worker.project_id}</span><span>Node: {observer.details.worker.node_id}</span><span class="col-span-2">HarnessInstance: {observer.details.worker.harness_instance_id}</span></div>{#if observer.details.approvals?.length}<div class="mt-4 space-y-2">{#each observer.details.approvals as approval}{#if approval.kind === 'input'}{@const request = inputRequest(approval)}<div class="rounded-xl border border-cyan-300/20 bg-cyan-300/[0.06] p-3 text-xs text-cyan-100"><p>Input required: {request?.prompt}</p>{#if request?.schema?.properties}<form class="mt-2 space-y-2" on:submit|preventDefault={() => submitInputRequest(request)}>{#each Object.entries(request.schema.properties) as [name, property]}<label class="block text-[11px] text-cyan-100">{property.title || name}{#if (request.schema.required || []).includes(name)} <span aria-hidden="true">*</span>{/if}{#if property.type === 'boolean'}<input type="checkbox" checked={inputFieldValue(request.requestId, name, property)} on:change={(event) => setInputField(request.requestId, name, property, event)} class="ml-2" aria-label={property.title || name} />{:else if property.type === 'array'}<select multiple on:change={(event) => setInputField(request.requestId, name, property, event)} class="mt-1 block min-h-16 w-full rounded-lg border border-white/10 bg-black/20 px-2 py-1 text-xs text-slate-100" aria-label={property.title || name}>{#each property.items?.enum || property.items?.anyOf?.map((item) => item.const) || [] as option}<option value={option}>{option}</option>{/each}</select>{:else if property.enum || property.oneOf}<select value={inputFieldValue(request.requestId, name, property)} on:change={(event) => setInputField(request.requestId, name, property, event)} class="mt-1 block w-full rounded-lg border border-white/10 bg-black/20 px-2 py-1 text-xs text-slate-100" aria-label={property.title || name}><option value="">Select...</option>{#each property.enum || property.oneOf.map((item) => item.const) as option}<option value={option}>{option}</option>{/each}</select>{:else}<input type={property.type === 'number' || property.type === 'integer' ? 'number' : property.format === 'email' ? 'email' : property.format === 'uri' ? 'url' : 'text'} value={inputFieldValue(request.requestId, name, property)} on:input={(event) => setInputField(request.requestId, name, property, event)} class="mt-1 block w-full rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-xs text-slate-100" aria-label={property.title || name} />{/if}</label>{/each}<button class="rounded-lg bg-cyan-300 px-3 py-1.5 text-xs text-slate-950" type="submit">Send</button></form>{:else}<form class="mt-2 flex gap-2" on:submit|preventDefault={() => workerAction('message', inputDrafts[request.requestId] || '', request.requestId)}><input value={inputDrafts[request.requestId] || ''} on:input={(event) => { inputDrafts = { ...inputDrafts, [request.requestId]: event.currentTarget.value }; }} class="min-w-0 flex-1 rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-xs text-slate-100" aria-label="Worker input" /><button class="rounded-lg bg-cyan-300 px-3 py-1.5 text-xs text-slate-950" type="submit">Send</button></form>{/if}</div>{:else}<div class="rounded-xl border border-amber-300/20 bg-amber-300/[0.06] p-3 text-xs text-amber-100"><p>{approval.action_summary}</p><button class="mt-2 rounded-lg bg-amber-300 px-3 py-1.5 text-xs text-slate-950" on:click={() => approve(approval)}>Approve</button></div>{/if}{/each}</div>{/if}<div class="mt-4 max-h-64 overflow-y-auto rounded-2xl bg-black/35 p-3 font-mono text-[11px] leading-5 text-emerald-200" aria-live="polite">{#if activityLines.length === 0}<span class="text-slate-600">No live activity yet.</span>{/if}{#each activityLines as item (item.id || item.seq)}<div class="border-b border-white/5 pb-2 last:border-0"><div class="text-slate-500">{item.kind}</div><pre class="mt-1 whitespace-pre-wrap break-words text-emerald-100">{formatActivityPayload(item)}</pre></div>{/each}</div><div class="mt-4 grid gap-2"><form class="flex gap-2" on:submit|preventDefault={() => workerAction('message', steerText)}><input bind:value={steerText} class="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/[0.06] px-3 py-2 text-xs text-slate-100" placeholder="Message active Worker" /><button class="rounded-xl bg-indigo-400 px-3 py-2 text-xs font-medium text-slate-950">Message</button></form><form class="flex gap-2" on:submit|preventDefault={() => workerAction('follow-up', followUpText)}><input bind:value={followUpText} class="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/[0.06] px-3 py-2 text-xs text-slate-100" placeholder="Follow-up for this Worker" /><button class="rounded-xl border border-white/10 px-3 py-2 text-xs text-slate-300">Follow-up</button></form></div><details class="mt-4 rounded-xl border border-white/10 p-3 text-xs text-slate-500"><summary class="cursor-pointer text-slate-400">Diagnostics</summary><div class="mt-3 space-y-3"><section><h3 class="text-slate-400">AttemptOutcomes</h3><div class="mt-2 space-y-1">{#each observer.diagnostics?.attempt_outcomes || observer.details.outcomes || [] as outcome}<div>{outcome.status} · {outcome.classification}{#if outcome.error_code} · {outcome.error_code}{/if}{#if outcome.error_message} · {outcome.error_message}{/if}</div>{/each}</div></section><section><h3 class="text-slate-400">Raw harness details</h3><div class="mt-2 max-h-48 space-y-2 overflow-y-auto">{#if !(observer.diagnostics?.raw_harness_details || []).length}<p class="text-slate-600">No harness details recorded.</p>{/if}{#each observer.diagnostics?.raw_harness_details || [] as detail}<div class="rounded-lg bg-black/20 p-2"><div class="text-slate-500">{detail.direction} · {detail.method || detail.session_update || 'record'}</div>{#if detail.tool}<div class="text-slate-400">tool: {detail.tool}{#if detail.status} · {detail.status}{/if}</div>{/if}{#if detail.details}<pre class="mt-1 whitespace-pre-wrap break-words text-slate-300">{JSON.stringify(detail.details, null, 2)}</pre>{/if}</div>{/each}</div></section></div></details>{#if observerError}<p class="mt-3 text-xs text-rose-300">{observerError}</p>{/if}{/if}</section>{/if}
    </div>
  </main>
{/if}
