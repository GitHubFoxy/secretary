...
func TestIdempotentWorkerCreationSurvivesReopenAndConcurrency(t *testing.T) {
	...
	spec := phase4WorkerSpec()
	spec.IdempotencyKey = "spawn-reopen"
	...
	for i := 0; i < 4; i++ {
		...
		go func() {
			defer wg.Done()
			worker, turn, attempt, err := store.CreateWorker(ctx, conversation.ID, spec, TurnSpec{Input: "same"})
			if err != nil {
				errs <- err
				return
			}
			results <- worker.ID + turn.ID + attempt.ID
		}()
	}
	wg.Wait()
	close(results)
	...
	for result := range results {
		if result != first.ID+firstTurn.ID+firstAttempt.ID {
	...
	if err := store.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM idempotency_records WHERE operation = 'worker.create'`).Scan(&idempotency); err != nil {
		t.Fatal(err)
	}
	if workers != 1 || turns != 1 || attempts != 1 || idempotency != 1 {
		t.Fatalf("counts workers=%d turns=%d attempts=%d idempotency=%d", workers, turns, attempts, idempotency)
	}
}
...
func TestReplayEventsPaginationHasNoLoss(t *testing.T) {
	...
	store := newTestStore(t)
	for i := 0; i < 501; i++ {
		if _, err := store.RecordEvent(ctx, "activity", "worker", "attempt", "legacy-session", map[string]int{"i": i}); err != nil {
			t.Fatal(err)
		}
	}
	...
	if len(page.Events) != 500 || !page.HasMore || page.LastReturnedSeq != page.Events[499].Seq || page.SnapshotBoundarySeq <= page.LastReturnedSeq {
	...
	if len(next.Events) != 1 || next.HasMore || next.Events[0].Seq != page.SnapshotBoundarySeq {
...
func TestRuntimeSessionIsNotExposedByServerEvents(t *testing.T) {
	...
	if string(encoded) == "runtime_session_id" || event.Payload == nil {
		t.Fatalf("event payload=%s", encoded)
	}
	var columns int
	if err := store.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM pragma_table_info('events') WHERE name = 'runtime_session_id'`).Scan(&columns); err != nil {
		t.Fatal(err)
	}
	if columns != 1 {
		t.Fatalf("legacy runtime column was removed, count=%d", columns)
	}
}
...
func TestFinalResultHasOneVisualDeliveryAndGenericDeliveryAudit(t *testing.T) {
	...
	_, turn, attempt, err := store.CreateWorker(ctx, conversation.ID, phase4WorkerSpec(), TurnSpec{Input: "result"})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := store.SetPhase4AttemptActive(ctx, attempt.ID); err != nil {
	...
	finished, err := store.FinishAttempt(ctx, attempt.ID, FinishAttemptInput{AttemptOutcomeInput: AttemptOutcomeInput{Status: OutcomeSucceeded, Classification: OutcomeFinal, Summary: "done"}})
	if err != nil {
		t.Fatal(err)
	}
	...
	deliveries, err := store.DeliveriesForEntry(ctx, entries[0].ID)
	if err != nil || len(deliveries) != 1 {
		t.Fatalf("deliveries=%#v err=%v", deliveries, err)
	}
	var resultAcceptedDeliveries int
	if err := store.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM deliveries d JOIN events e ON e.id = d.event_id WHERE e.kind = 'result.accepted' AND e.aggregate_id = ?`, finished.Result.ID).Scan(&resultAcceptedDeliveries); err != nil {
		t.Fatal(err)
	}
	if resultAcceptedDeliveries != 0 {
		t.Fatalf("result.accepted had visual delivery: %d", resultAcceptedDeliveries)
	}
	if _, err := store.MarkDeliveryDelivered(ctx, deliveries[0].ID); err != nil {
		t.Fatal(err)
	}
	var auditKind string
	if err := store.db.QueryRowContext(ctx, `SELECT kind FROM events WHERE aggregate_id = ? ORDER BY seq DESC LIMIT 1`, deliveries[0].EventID).Scan(&auditKind); err != nil {
		t.Fatal(err)
	}
	if auditKind != "delivery.delivered" {
		t.Fatalf("audit kind=%q", auditKind)
	}
	if turn.ID == "" {
...
func TestUnmarshalableEventPayloadAbortsTransaction(t *testing.T) {
	ctx := context.Background()
	store := newTestStore(t)
	before, err := store.ReplayEvents(ctx, 0, 500)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := store.RecordEventWithMetadata(ctx, EventInput{Kind: "bad", Payload: func() {}}); err == nil {
		t.Fatal("unmarshalable payload was accepted")
	}
	after, err := store.ReplayEvents(ctx, 0, 500)
	if err != nil {
		t.Fatal(err)
	}
	if len(after.Events) != len(before.Events) || after.SnapshotBoundarySeq != before.SnapshotBoundarySeq {
		t.Fatalf("failed payload mutated durable state: before=%#v after=%#v", before, after)
	}
}
