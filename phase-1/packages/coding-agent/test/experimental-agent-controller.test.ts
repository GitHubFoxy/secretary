...
describe("AgentController service", () => {
	test("provides the presentation-safe AgentLane command surface", async () => {
		...
		const watch = vi.fn(async () => ({ snapshot: { operation: { id: "operation-current" } }, unsubscribe }));
		const lane = { prompt, requestAbort, steer, followUp, watch } as unknown as AgentLane;
		...
		try {
			...
			await expect(
				host.services.invoke(
					{ serviceId: AgentController.id, member: "abortCurrent", args: [] },
					BACKGROUND_CONTEXT,
				),
			).resolves.toBeUndefined();
			await expect(
			...
			expect(requestAbort).toHaveBeenCalledWith("operation-current", BACKGROUND_CONTEXT);
