...
import { createExtensionUIBridge } from "../src/experimental/full-worker-resources.ts";
	...
	test.each([
		...
		async (kind, command, sessionId, creates) => {
			...
			const requestAbort = vi.fn(async () => ({
				ok: true as const,
				value: { operationId: "run-1", newlyRequested: true, steer: [], followUp: [] },
			}));
			const watch = vi.fn(async () => ({
				snapshot: transcriptState.value.snapshot!,
				unsubscribe: vi.fn(),
			}));
			...
			const extensionCommandRun = vi.fn(async () => ["Fixture accepted."]);
			let holdEscapeInput = false;
			let releaseEscapeInput: (() => void) | undefined;
			let useCustomEditor = false;
			let editorBridge: ReturnType<typeof createExtensionUIBridge>;
			const inputUI = vi.fn(async (id: string, data: string) => {
				if (useCustomEditor) return editorBridge.input(id, data);
				if (holdEscapeInput && data.includes("\u001b")) {
					await new Promise<void>((resolve) => {
						releaseEscapeInput = resolve;
					});
				}
				return false;
			});
			const uiState = replicatedState<ExtensionUIState>({
				revision: 0,
				prompt: null,
				statuses: {},
				notifications: [],
			...
			editorBridge = createExtensionUIBridge("/tmp");
			editorBridge.attachState(uiState);
			...
			sessionProvider.provide(ExtensionCommands, {
				async list() {
					return [{ name: "fixture", description: "Worker command" }];
				},
				async complete() {
					return null;
				},
				async autocomplete() {
					return { suggestions: null, completions: {} };
				},
				async getAutocompleteTriggerCharacters() {
					return [];
				},
				run: extensionCommandRun,
				uiState,
				async respondUI(id, value) {
					await editorBridge.respond(id, value);
				},
				inputUI,
				async setToolsExpanded() {},
			});
			...
			sessionProvider.provide(
				AgentController,
				createAgentController({ prompt, requestAbort, watch } as unknown as AgentLane),
			);
			sessionProvider.provide(Transcript, {
				state: transcriptState,
				async snapshot() {
					return transcriptState.state.snapshot!;
				},
			});
			...
			const sessionNamespace = createRemoteServiceBinding({
				services: [Models, AgentController, ExtensionCommands, SessionPlugins, Transcript],
			...
			try {
				...
				holdEscapeInput = true;
				publishReplacement(uiState, { ...uiState.value!, terminalInput: true });
				component.handleInput("\u001b");
				await vi.waitFor(() => expect(requestAbort).toHaveBeenCalledWith("run-1", BACKGROUND_CONTEXT));
				holdEscapeInput = false;
				publishReplacement(uiState, { ...uiState.value!, terminalInput: undefined });
				releaseEscapeInput?.();
				finishPrompt();
				...
				component.handleInput("/fixture accepted");
				component.handleInput("\r");
				await vi.waitFor(() =>
					expect(extensionCommandRun).toHaveBeenCalledWith("fixture", "accepted", expect.anything()),
				);
				await vi.waitFor(() => expect(component.render(80).join("\n")).toContain("Fixture accepted."));

				useCustomEditor = true;
				let workerEditor: CustomEditor | undefined;
				editorBridge.ui.setEditorComponent((editorUi, editorTheme, keybindings) => {
					workerEditor = new CustomEditor(editorUi, editorTheme, keybindings);
					workerEditor.setAutocompleteProvider({
				...
				await vi.waitFor(() => expect(component.render(80).join("\n")).toContain("────────────────"));
				const confirmation = editorBridge.ui.confirm("Confirm custom editor", "Escape must dismiss this dialog.");
				await vi.waitFor(() => expect(component.render(80).join("\n")).toContain("Confirm custom editor"));
				component.handleInput("\u001b");
				await expect(confirmation).resolves.toBe(false);
				await vi.waitFor(() => expect(component.render(80).join("\n")).not.toContain("Confirm custom editor"));
				const inputPrompt = editorBridge.ui.input("Skill profile", "Profile name");
				await vi.waitFor(() => expect(component.render(80).join("\n")).toContain("Profile name"));
				component.handleInput("\u001b");
				await expect(inputPrompt).resolves.toBeUndefined();
				component.handleInput("/fixture from-custom-editor");
				component.handleInput("\r");
				await vi.waitFor(() =>
					expect(extensionCommandRun).toHaveBeenCalledWith("fixture", "from-custom-editor", expect.anything()),
				);
				emitTranscriptEvent({ type: "run_start", lane: "main", runId: "run-2", startedAt: 3 });
				component.handleInput("\t");
				await vi.waitFor(() => expect(workerEditor!.isShowingAutocomplete()).toBe(true));
				component.handleInput("\u001b");
				await vi.waitFor(() => expect(workerEditor!.isShowingAutocomplete()).toBe(false));
				expect(requestAbort).not.toHaveBeenCalledWith("run-2", BACKGROUND_CONTEXT);
				component.handleInput("\u001b");
				await vi.waitFor(() => expect(requestAbort).toHaveBeenCalledWith("run-2", BACKGROUND_CONTEXT));
				emitTranscriptEvent({
					type: "run_end",
					lane: "main",
					runId: "run-2",
				...
				editorBridge.ui.setEditorComponent(undefined);
