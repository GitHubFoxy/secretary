...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
	...
	void runSessionWorkerWithHarness(process.argv.slice(2), async (session, options) => {
		...
		faux.setResponses([
			process.env.PI_FAUX_REMOTE_QUOTA_ERROR === "1"
				? fauxAssistantMessage("", { stopReason: "error", errorMessage: "insufficient_quota", timestamp: 20 })
				: fauxAssistantMessage("deterministic remote answer", { timestamp: 20 }),
		]);
