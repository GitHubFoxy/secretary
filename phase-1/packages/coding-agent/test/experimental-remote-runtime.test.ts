...
describe("experimental durable server composition", () => {
	...
	test("reconnects a Unix client and restores its selected Session", async () => {
		const { runtime } = await makeServer();
		...
		try {
			const services = await activateBuiltinClientServices(clientRuntime.servers[0]!);
			await services.management.attach("demo-1", BACKGROUND_CONTEXT);
			const workerPid = runtime.workerPids.get("demo-1");
			expect(workerPid).toEqual(expect.any(Number));
			clientRuntime.servers[0]!.client.disconnect("fixture interruption");
			await vi.waitFor(() => expect(clientRuntime.servers[0]!.client.connected).toBe(true), { timeout: 5_000 });
			await vi.waitFor(() => expect(clientRuntime.servers[0]!.client.attachment?.sessionId).toBe("demo-1"), {
				timeout: 5_000,
			});
			const snapshot = await services.transcript.snapshot(BACKGROUND_CONTEXT);
			expect(snapshot.transcript).toBeDefined();
			expect(runtime.workerPids.size).toBe(1);
	...
	test("replicates a worker-provider quota failure to the remote client", async ({ onTestFinished }) => {
		vi.stubEnv("PI_FAUX_REMOTE_QUOTA_ERROR", "1");
		const spawn = vi
			.spyOn(processRuntime, "spawnInternalProcess")
			.mockImplementation((role, args, options) =>
				realSpawnInternalProcess(
		...
		const { runtime } = await makeServer();
		const client = await attachClient(runtime, "demo-1");
		const services = createSessionServiceBinding(client, { services: [AgentController, Transcript] });
		const controller = services.use(AgentController);
		const transcript = services.use(Transcript);
		await services.ready(BACKGROUND_CONTEXT);
		try {
			const response = await controller.prompt({ message: "quota fixture", images: null }, BACKGROUND_CONTEXT);
			await vi.waitFor(() =>
			...
			const snapshot = await transcript.snapshot(BACKGROUND_CONTEXT);
			expect(snapshot.transcript).toContainEqual(
				expect.objectContaining({
					type: "message",
					message: expect.objectContaining({ role: "assistant", errorMessage: "insufficient_quota" }),
				}),
			);
			expect(runtime.workerPids).toHaveLength(1);
		} finally {
			await services.dispose(BACKGROUND_CONTEXT);
		}
	});
	...
	test("attaches a second client to a large replicated transcript without creating another worker", async ({
		onTestFinished,
	}) => {
		const spawn = vi
			.spyOn(processRuntime, "spawnInternalProcess")
		...
		onTestFinished(() => spawn.mockRestore());
		const { runtime } = await makeServer();
		const first = await attachClient(runtime, "demo-1");
		const firstServices = createSessionServiceBinding(first, { services: [AgentController, Transcript] });
		const controller = firstServices.use(AgentController);
		const firstTranscript = firstServices.use(Transcript);
		await firstServices.ready(BACKGROUND_CONTEXT);
		try {
			const payload = "large remote transcript ".repeat(512);
			for (let index = 0; index < 16; index++) {
				const response = await controller.prompt(
					{ message: `${index}:${payload}`, images: null },
					BACKGROUND_CONTEXT,
				);
				await vi.waitFor(() =>
					expect(firstTranscript.state.value?.snapshot?.lastResult).toMatchObject({
						operationId: response.operationId,
					}),
				);
			}
			const second = await attachClient(runtime, "demo-1");
			const secondServices = createSessionServiceBinding(second, { services: [Transcript] });
			await secondServices.ready(BACKGROUND_CONTEXT);
			try {
				const snapshot = await secondServices.use(Transcript).snapshot(BACKGROUND_CONTEXT);
				expect(snapshot.transcript.length).toBeGreaterThanOrEqual(32);
				expect(runtime.workerPids).toHaveLength(1);
			} finally {
				await secondServices.dispose(BACKGROUND_CONTEXT);
			}
