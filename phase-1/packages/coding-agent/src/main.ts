...
async function runClientCommand(command: ClientCommand): Promise<void> {
	...
	const result = await runClient(command, {
		onSessionCreated(session) {
			process.stdout.write(`THREAD_ID=${session.sessionId}\n`);
		},
		onEvent(event) {
	...
	if (result.kind === "queued") {
		console.log(`${result.serverId}\t${result.sessionId}\tqueued\t${result.entryId}`);
		return;
	}
	if (result.kind === "aborted") {
		console.log(`${result.serverId}\t${result.sessionId}\taborted\t${result.operationId}`);
		return;
	}
...
async function runExperimentalCommand(args: string[]): Promise<boolean> {
	...
	try {
		...
		if (error instanceof AggregateError) {
			for (const nested of error.errors) messages.push(nested instanceof Error ? nested.message : String(nested));
		}
