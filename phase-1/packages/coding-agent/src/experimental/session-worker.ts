...
import { loadFullWorkerResources } from "./full-worker-resources.ts";
...
export const SessionWorkerOptionsSchema = StrictObject({
	...
	workerDepth: Type.Optional(Type.Integer({ minimum: 1 })),
});
...
async function run(options: SessionWorkerOptions, createHarness: CreateSessionWorkerHarness): Promise<void> {
	...
	try {
		...
		services = await createSessionWorkerServices({
			...
			reloadResources: runtime.reloadResources,
			extensionCommands: runtime.extensionCommands,
...
async function createCodingAgentHarness(
	...
	let resources = await loadFullWorkerResources(
		session.metadata.cwd,
		(options.workerDepth ?? 1) < 2,
		undefined,
		modelRuntime,
	);
	if (resources.diagnostics.length > 0) {
		console.error(resources.diagnostics.map((diagnostic) => `Resource warning: ${diagnostic}`).join("\\n"));
	}
	...
	const requestedThinking = process.env.PI_SUBPI_THINKING;
	const thinkingLevel =
		...
		await AgentHarness.create(
			{
				...
				activeToolNames: resources.activeToolNames,
	...
	try {
		...
		if (
			currentActiveToolNames.length !== resources.activeToolNames.length ||
			currentActiveToolNames.some((name, index) => name !== resources.activeToolNames[index])
		) {
			await lane.setActiveTools(resources.activeToolNames, TODO_CONTEXT);
		}
		let deactivate = await resources.activate({ harness, lane, metadata: session.metadata });
		return {
			harness,
			lane,
			modelRuntime,
			...
			extensionCommands: {
				list: () => Promise.resolve(resources.commands.list()),
				complete: (name, prefix) => resources.commands.complete(name, prefix),
				run: (name, args) => resources.commands.run(name, args),
			},
			async reloadResources() {
				const replacement = await loadFullWorkerResources(
					session.metadata.cwd,
					(options.workerDepth ?? 1) < 2,
					resources.extensionUI,
					modelRuntime,
				);
				await deactivate();
				resources = replacement;
				await harness.setTools(replacement.tools, TODO_CONTEXT);
				await harness.setResources(replacement.resources, TODO_CONTEXT);
				await lane.setActiveTools(replacement.activeToolNames, TODO_CONTEXT);
				deactivate = await replacement.activate({ harness, lane, metadata: session.metadata, reason: "reload" });
			},
		};
	} catch (error) {
		try {
