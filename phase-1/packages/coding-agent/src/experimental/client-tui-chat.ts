...
import type { EntryRenderer, MessageRenderer, WorkingIndicatorOptions } from "../core/extensions/types.ts";
...
import { CustomEntryComponent } from "../modes/interactive/components/custom-entry.ts";
...
export class ExperimentalChatView {
	readonly #renderers: Record<string, ToolRenderers>;
	...
	#working = false;
	#workingMessage = "Working... (esc to abort)";
	#workingVisible = true;
	#workingIndicator: WorkingIndicatorOptions | undefined;
	...
	constructor(
		...
		messageRenderers: ReadonlyMap<string, MessageRenderer> = new Map(),
		entryRenderers: ReadonlyMap<string, EntryRenderer> = new Map(),
	) {
		this.#ui = ui;
		this.#cwd = cwd;
		this.#renderers = { ...createAllToolRenderers(), ...extensionRenderers };
	...
	setHiddenThinkingLabel(label?: string): void {
		...
		this.#streaming?.setHiddenThinkingLabel(this.#hiddenThinkingLabel);
		this.transcript.invalidate();
	}

	setToolsExpanded(expanded?: boolean): void {
		this.#toolsExpanded = expanded ?? false;
		for (const tool of this.#tools.values()) tool.setExpanded(this.#toolsExpanded);
	}

	setWorkingPresentation(options: { message?: string; visible: boolean; indicator?: WorkingIndicatorOptions }): void {
		this.#workingMessage = options.message ?? "Working... (esc to abort)";
		this.#workingVisible = options.visible;
		this.#workingIndicator = options.indicator;
		if (this.#working) {
			...
			if (renderer) {
				this.transcript.addChild(
					new CustomEntryComponent(
						{ ...entry, timestamp: new Date(entry.timestamp).toISOString() } as never,
						renderer,
					),
				);
			} else {
	...
	#addMessage(message: AgentMessage): void {
		...
		if (message.role === "assistant") {
			const component =
				this.#streaming ??
				new AssistantMessageComponent(undefined, false, getMarkdownTheme(), this.#hiddenThinkingLabel);
		...
		if (message.role === "custom" && message.display) {
			this.transcript.addChild(
				new CustomMessageComponent(message, this.#messageRenderers.get(message.customType), getMarkdownTheme(), 1),
			);
		}
	}
	...
	#setWorking(working: boolean): void {
		...
		this.#indicator = undefined;
		this.status.clear();
		if (working && this.#workingVisible) {
			this.#indicator = new WorkingStatusIndicator(this.#ui, this.#workingMessage, this.#workingIndicator);
