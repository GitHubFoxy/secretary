...
async function startServerBackend(
	...
	const workerDepthPath = (metadata: JsonlSessionMetadata): string => `${metadata.path}.worker-depth`;
	const readWorkerDepth = async (metadata: JsonlSessionMetadata): Promise<number> => {
		try {
			const value = Number((await readFile(workerDepthPath(metadata), "utf8")).trim());
	...
	const createSession = async (
		...
		const { workerDepth = 1, ...repoOptions } = createOptions;
		const session = await repo.create({ ...repoOptions, cwd: createOptions.cwd ?? process.cwd() }, context);
		try {
			await writeFile(workerDepthPath(session.metadata), `${workerDepth}\n`, "utf8");
	...
	const host: ServerHost<JsonlSessionMetadata> = {
		...
		openSession: async (metadata, context) => {
			...
			return workers.openSession(metadata, context, selected.manifestPaths, await readWorkerDepth(metadata));
		},
	};
