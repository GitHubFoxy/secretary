...
export function createTranscriptService(
	...
	const publishSnapshot = (
		...
		state.state.snapshot = jsonSafe(next) as LaneTranscriptSnapshot;
		state.state.event = event === null ? null : jsonSafe(event);
	...
	const onEvent = (event: HarnessEvent, context: Parameters<typeof state.publish>[0]): void => {
		...
		const safeEvent = jsonSafe(forwarded);
		const snapshot = state.state.snapshot;
		if (snapshot === null) throw new Error("Transcript service is not active");
		if (reduceLaneSnapshot(snapshot, safeEvent) === "rebase") scheduleRebase(context);
		state.state.event = safeEvent;
		state.publish(context);
	};
	...
	return {
		service: {
			...
			async snapshot(context) {
				if (watch === undefined) throw new Error("Transcript service is not active");
				return jsonSafe(await watch.resnapshot(context)) as LaneTranscriptSnapshot;
			},
		},
		async activate() {
