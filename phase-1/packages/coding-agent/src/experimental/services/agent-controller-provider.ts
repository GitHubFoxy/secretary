import type { AgentHarness, AgentLane, OperationResultRecord, SuspendedRun } from "@earendil-works/pi-agent-core";
...
export function createAgentController(harness: AgentHarness, lane: AgentLane): AgentControllerService;
export function createAgentController(
	harnessOrLane: AgentHarness | AgentLane,
	suppliedLane?: AgentLane,
): AgentControllerService {
	const harness = suppliedLane === undefined ? undefined : (harnessOrLane as AgentHarness);
	const lane = suppliedLane ?? (harnessOrLane as AgentLane);
	const queue = async (
	...
	return {
		async getName(context) {
			return harness === undefined ? null : ((await harness.getName(context)) ?? null);
		},
		async setName(name, context) {
			if (harness === undefined) throw new Error("Session naming is unavailable without an AgentHarness");
			await harness.setName(name ?? undefined, context);
		},
		...
		async abortCurrent(context) {
			const watch = await lane.watch(context);
			try {
				const operation = watch.snapshot.operation;
				if (operation === null) return;
				const result = await lane.requestAbort(operation.id, context);
				if (!result.ok && result.error._tag !== "OperationMismatch") throw new Error(result.error.message);
			} finally {
				watch.unsubscribe();
			}
		},
